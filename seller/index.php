...
// --- 5. DATA RINCIAN NOTIFIKASI ---
$q_pending_list = mysqli_query($conn, "
    SELECT DISTINCT o.invoice_number, u.full_name, o.order_date
    FROM orders o
    JOIN order_items oi ON o.id = oi.order_id
    JOIN users u ON o.buyer_id = u.id
...
$q_unread_list = mysqli_query($conn, "
    SELECT m.message, u.full_name, m.created_at, m.sender_id
    FROM messages m
    JOIN users u ON m.sender_id = u.id
...
<div class="flex min-h-screen">
    ...
    <aside id="sidebar" class="w-64 bg-white dark:bg-stone-900 border-r border-[var(--border-color)] flex flex-col fixed h-full z-30 overflow-hidden shadow-lg lg:shadow-none">
    ...
    <main id="main-content" class="flex-1 ml-64 p-4 lg:p-8 transition-all duration-300">
        ...
        <header class="flex justify-between items-center mb-8 bg-white/50 dark:bg-stone-900/50 backdrop-blur-sm p-4 rounded-3xl border border-[var(--border-color)] sticky top-4 z-20 shadow-sm" data-aos="fade-down">
            <div class="flex items-center gap-4">
                <button onclick="toggleSidebar()" class="p-2 rounded-xl hover:bg-[var(--light-sage)] text-[var(--deep-forest)] transition-colors focus:outline-none dark:text-stone-300 dark:hover:bg-stone-800">
            ...
            <div class="flex items-center gap-4 relative">
                <button id="dark-mode-icon" onclick="toggleDarkMode()" class="w-10 h-10 rounded-full bg-white dark:bg-stone-800 border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--deep-forest)] hover:shadow-md transition-all cursor-pointer">
                    dark_mode
                </button>

                <button onclick="toggleDropdown('notificationDropdown')" class="w-10 h-10 rounded-full bg-white dark:bg-stone-800 border border-[var(--border-color)] flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--deep-forest)] hover:shadow-md transition-all relative">
                ...
                <div id="notificationDropdown" class="absolute right-16 top-14 w-80 bg-white dark:bg-stone-800 rounded-2xl shadow-xl border border-[var(--border-color)] py-2 hidden transform origin-top-right transition-all z-50">
                    <div class="px-4 py-3 border-b border-gray-100 dark:border-stone-700 flex justify-between items-center">
                        <h4 class="font-bold text-[var(--deep-forest)] dark:text-stone-200">Notifikasi</h4>
                        <?php if($total_notif > 0): ?>
                            <span class="text-[10px] bg-red-100 text-red-600 px-2 py-1 rounded-full font-bold"><?= $total_notif ?> Baru</span>
                        <?php endif; ?>
                    </div>
                    <div class="max-h-64 overflow-y-auto">
                        <?php if(mysqli_num_rows($q_pending_list) > 0): ?>
                            <div class="px-4 py-2 bg-gray-50 dark:bg-stone-700/50 text-[10px] font-bold text-gray-500 dark:text-stone-400 uppercase tracking-wider">Pesanan Masuk</div>
                            <?php while($n = mysqli_fetch_assoc($q_pending_list)): ?>
                            <a href="orders.php" class="flex items-start gap-3 px-4 py-3 hover:bg-[var(--cream-bg)] dark:hover:bg-stone-700 transition-colors border-b border-gray-50 dark:border-stone-700">
                                <div class="p-2 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 rounded-full shrink-0"><span class="material-symbols-outlined text-lg">shopping_bag</span></div>
                                <div>
                                    <p class="text-sm font-bold text-gray-800 dark:text-stone-200 line-clamp-1">Order #<?= $n['invoice_number'] ?></p>
                                    <p class="text-xs text-gray-500 dark:text-stone-400">Dari: <?= $n['full_name'] ?></p>
                                    <p class="text-[10px] text-gray-400 dark:text-stone-500 mt-1"><?= date('d M H:i', strtotime($n['order_date'])) ?></p>
                                </div>
                            </a>
                            <?php endwhile; ?>
                        <?php endif; ?>
                        
                        <?php if(mysqli_num_rows($q_unread_list) > 0): ?>
                            <div class="px-4 py-2 bg-gray-50 dark:bg-stone-700/50 text-[10px] font-bold text-gray-500 dark:text-stone-400 uppercase tracking-wider">Chat Baru</div>
                            <?php while($c = mysqli_fetch_assoc($q_unread_list)): ?>
                            <a href="chat.php?uid=<?= $c['sender_id'] ?>" class="flex items-start gap-3 px-4 py-3 hover:bg-[var(--cream-bg)] dark:hover:bg-stone-700 transition-colors border-b border-gray-50 dark:border-stone-700">
                                <div class="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full shrink-0"><span class="material-symbols-outlined text-lg">chat</span></div>
                                <div>
                                    <p class="text-sm font-bold text-gray-800 dark:text-stone-200"><?= $c['full_name'] ?></p>
                                    <p class="text-xs text-gray-500 dark:text-stone-400 line-clamp-1"><?= $c['message'] ?></p>
                                    <p class="text-[10px] text-gray-400 dark:text-stone-500 mt-1"><?= date('H:i', strtotime($c['created_at'])) ?></p>
                                </div>
                            </a>
                            <?php endwhile; ?>
                        <?php endif; ?>
                        ...
                        <?php if($total_notif == 0): ?>
                            <div class="text-center py-6 text-gray-400 dark:text-stone-500 text-xs italic">Tidak ada notifikasi baru.</div>
                        <?php endif; ?>
                    </div>
                </div>

                <button onclick="toggleDropdown('profileDropdown')" class="flex items-center gap-3 bg-white dark:bg-stone-800 p-1.5 pr-4 rounded-full border border-[var(--border-color)] card-shadow hover:shadow-md transition-all focus:outline-none">
                    <div class="w-9 h-9 rounded-full bg-[var(--warm-tan)] text-white flex items-center justify-center font-bold text-sm border-2 border-[var(--cream-bg)] dark:border-stone-700"><?= strtoupper(substr($seller_name, 0, 1)) ?></div>
                ...
                <div id="profileDropdown" class="absolute right-0 top-14 w-56 bg-white dark:bg-stone-800 rounded-2xl shadow-xl border border-[var(--border-color)] py-2 hidden transform origin-top-right transition-all z-50">
                    <a href="profile.php" class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 dark:text-stone-300 hover:bg-[var(--light-sage)]/30 hover:text-[var(--deep-forest)] transition-colors"><span class="material-symbols-outlined text-[20px]">store</span> Profil Toko</a>
                    <div class="border-t border-gray-100 dark:border-stone-700 my-1"></div>
                    <a href="../auth/logout.php" class="flex items-center gap-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-stone-700 transition-colors"><span class="material-symbols-outlined text-[20px]">logout</span> Log Out</a>
                </div>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8" data-aos="fade-up" data-aos-delay="100">
            <div class="bg-white dark:bg-stone-900 p-6 rounded-[2.5rem] border border-[var(--border-color)] card-shadow flex flex-col gap-2 hover:-translate-y-1 transition-transform group">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-[var(--deep-forest)] dark:text-stone-200 mt-1">Rp <?= number_format($revenue, 0, ',', '.') ?></h3>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center text-green-700 dark:text-green-300 group-hover:scale-110 transition-transform"><span class="material-symbols-outlined text-2xl">payments</span></div>
                </div>
                <div class="w-full bg-gray-100 dark:bg-stone-800 h-1.5 rounded-full mt-2 overflow-hidden">
            ...
            <div class="bg-white dark:bg-stone-900 p-6 rounded-[2.5rem] border border-[var(--border-color)] card-shadow flex flex-col gap-2 hover:-translate-y-1 transition-transform group">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-[var(--deep-forest)] dark:text-stone-200 mt-1"><?= $pending_count ?></h3>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-300 group-hover:scale-110 transition-transform"><span class="material-symbols-outlined text-2xl">shopping_cart</span></div>
                </div>
                <div class="w-full bg-gray-100 dark:bg-stone-800 h-1.5 rounded-full mt-2 overflow-hidden">
            ...
            <div class="bg-white dark:bg-stone-900 p-6 rounded-[2.5rem] border border-[var(--border-color)] card-shadow flex flex-col gap-2 hover:-translate-y-1 transition-transform group">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-[var(--deep-forest)] dark:text-stone-200 mt-1"><?= $product_count ?></h3>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-orange-700 dark:text-orange-300 group-hover:scale-110 transition-transform"><span class="material-symbols-outlined text-2xl">inventory_2</span></div>
                </div>
                <div class="w-full bg-gray-100 dark:bg-stone-800 h-1.5 rounded-full mt-2 overflow-hidden">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="lg:col-span-2 bg-white dark:bg-stone-900 rounded-[2.5rem] p-8 border border-[var(--border-color)] card-shadow">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h3 class="text-xl font-bold text-[var(--text-dark)] dark:text-stone-200 title-font">Statistik Penjualan</h3>
                        <p class="text-xs text-[var(--text-muted)]">Performa pendapatan tahun ini</p>
                    </div>
                    <span class="text-xs bg-[var(--cream-bg)] dark:bg-stone-800 px-3 py-1 rounded-lg font-bold text-[var(--deep-forest)] dark:text-stone-300 border border-[var(--border-color)]"><?= date('Y') ?></span>
                </div>
            ...
            <div class="lg:col-span-1 bg-white dark:bg-stone-900 rounded-[2.5rem] p-8 border border-[var(--border-color)] card-shadow relative overflow-hidden">
                <div class="flex items-center gap-2 mb-6">
                    <span class="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                    <h3 class="text-lg font-bold text-[var(--text-dark)] dark:text-stone-200 title-font">Stok Menipis</h3>
                </div>
                ...
                <div class="space-y-4">
                    <?php if(mysqli_num_rows($q_low_stock) > 0): ?>
                        ...
                        ?>
                        <div class="flex items-center gap-3 p-3 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30">
                            ...
                            <a href="products.php" class="p-1.5 bg-white dark:bg-stone-800 text-red-500 dark:text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all shadow-sm">
                    ...
                    <?php else: ?>
                        <div class="text-center py-8">
                            ...
                            <p class="text-sm font-bold text-green-700 dark:text-green-400">Stok Aman!</p>
                            <p class="text-xs text-gray-500 dark:text-stone-500">Tidak ada produk dengan stok rendah.</p>
                        </div>
                    <?php endif; ?>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-white dark:bg-stone-900 rounded-[2.5rem] p-8 border border-[var(--border-color)] card-shadow" data-aos="fade-up" data-aos-delay="300">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-[var(--text-dark)] dark:text-stone-200 title-font">Pesanan Terbaru</h3>
                    <a href="orders.php" class="text-xs font-bold text-[var(--deep-forest)] dark:text-stone-400 hover:underline flex items-center gap-1">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-[10px] text-[var(--text-muted)] uppercase border-b border-[var(--border-color)] tracking-wider dark:text-stone-500">
                        ...
                        <tbody class="text-sm dark:text-stone-300">
                            <?php if(mysqli_num_rows($q_recent) > 0): ?>
                                ...
                                ?>
                                <tr class="hover:bg-[var(--cream-bg)]/30 dark:hover:bg-stone-800 transition-colors border-b border-gray-50 dark:border-stone-800 last:border-0 cursor-pointer" onclick="window.location='orders.php'">
                                    <td class="px-4 py-3 font-mono font-bold text-[var(--deep-forest)] dark:text-stone-300 text-xs"><?= $order['invoice_number'] ?></td>
