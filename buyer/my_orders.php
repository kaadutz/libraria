...
// --- LOGIKA UPLOAD BUKTI BAYAR (DIPERBAIKI) ---
    ...
    if (!empty($_FILES['payment_proof']['name'])) {
        ...
        if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp', 'pdf'])) {
            if(move_uploaded_file($_FILES['payment_proof']['tmp_name'], $target_dir . $new_name)) {
                ...
                // 1. CEK DULU STATUS TERAKHIR DI DATABASE
                $cek_status = mysqli_query($conn, "SELECT status FROM orders WHERE id = '$order_id'");
                $current_data = mysqli_fetch_assoc($cek_status);
                $curr_status = $current_data['status'];

                // 2. LOGIKA UPDATE PINTAR
                // Hanya ubah jadi 'waiting_approval' kalau statusnya masih 'pending', 'rejected' atau 'waiting_approval'
                if ($curr_status == 'pending' || $curr_status == 'rejected' || $curr_status == 'waiting_approval') {
                    $query_up = "UPDATE orders SET payment_proof = '$new_name', status = 'waiting_approval' WHERE id = '$order_id'";
                } else {
                    // Kalau status sudah 'approved', 'shipping', atau 'finished', JANGAN ubah statusnya (Hanya ganti gambarnya saja)
                    $query_up = "UPDATE orders SET payment_proof = '$new_name' WHERE id = '$order_id'";
                }

                if(mysqli_query($conn, $query_up)) {
                    // Redirect supaya kalau di-refresh tidak upload ulang (Post-Redirect-Get Pattern)
                    header("Location: my_orders.php?status=success_upload"); 
                    exit; 
                }
            } else {
...
// --- LOGIKA KONFIRMASI TERIMA BARANG ---
...
if (isset($_POST['request_refund'])) {
    ...
    $success_msg = "Pengajuan Refund dikirim. Silakan hubungi Admin.";
}

// --- LOGIKA KONFIRMASI REFUND SELESAI (UANG DITERIMA) ---
if (isset($_POST['confirm_refund'])) {
    $order_id = $_POST['order_id'];
    mysqli_query($conn, "UPDATE orders SET status = 'refunded' WHERE id = '$order_id'");
    $success_msg = "Refund selesai. Pesanan ditutup.";
}
...
<body class="overflow-x-hidden min-h-screen flex flex-col">
    ...
    <div id="toast" class="fixed top-28 right-5 z-[60] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl text-white bg-[var(--deep-forest)] animate-bounce">
        ...
        <p class="text-sm font-bold"><?= $success_msg ?></p>
    </div>
    <?php endif; ?>
    
    <?php if(isset($error_msg)): ?>
    <div id="toast" class="fixed top-28 right-5 z-[60] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl text-white bg-red-600 animate-bounce">
        <span class="material-symbols-outlined">error</span>
        <p class="text-sm font-bold"><?= $error_msg ?></p>
    </div>
    <?php endif; ?>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative">
                        ...
                        <div id="notificationDropdown" class="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-xl border border-[var(--border-color)] py-2 hidden transform origin-top-right transition-all z-50">
                            <div class="px-4 py-3 border-b border-[var(--border-color)] flex justify-between items-center"><h4 class="font-bold text-[var(--deep-forest)] text-sm">Notifikasi</h4></div>
        ...
        <div class="space-y-6" data-aos="fade-up">
            ...
            <?php if(mysqli_num_rows($query_orders) > 0): ?>
                <?php while($order = mysqli_fetch_assoc($query_orders)): 
                    $order_id = $order['id'];
                    $items_q = mysqli_query($conn, "SELECT oi.*, b.title, b.image FROM order_items oi JOIN books b ON oi.book_id = b.id WHERE oi.order_id = '$order_id'");
                    ...
                    if($order['status'] == 'pending' || $order['status'] == 'waiting_approval') {
                        ...
                        $status_label = 'Pesanan Ditolak';
                    } elseif($order['status'] == 'refund') {
                        $status_class = 'bg-orange-100 text-orange-700';
                        $status_label = 'Proses Refund';
                    } elseif($order['status'] == 'refunded') {
                        $status_class = 'bg-stone-200 text-stone-600 line-through';
                        $status_label = 'Refund Selesai';
                    }
                ?>
                <div class="bg-white rounded-[2.5rem] p-6 border border-[var(--border-color)] shadow-sm hover:shadow-lg transition-all relative overflow-hidden">
                    
                    <?php if($order['status'] == 'refund'): ?>
                        <div class="absolute top-0 right-0 bg-orange-500 text-white text-[10px] font-bold px-4 py-1 rounded-bl-xl">Dana Sedang Diproses</div>
                    <?php endif; ?>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-dashed border-[var(--border-color)] pb-4 mb-4 gap-4">
                        <div>
                            <div class="flex items-center gap-3 mb-1">
                                ...
                                <h3 class="font-bold text-[var(--text-dark)] text-lg"><?= $order['invoice_number'] ?></h3>
                            </div>
                            <p class="text-xs text-[var(--text-muted)]"><?= date('d F Y, H:i', strtotime($order['order_date'])) ?></p>
                        </div>
                    ...
                    <div class="bg-gray-50 rounded-2xl p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div class="text-left w-full md:w-auto">
                            <p class="text-xs text-[var(--text-muted)] uppercase">Total Belanja</p>
                            <p class="text-xl font-bold text-[var(--chocolate-brown)]">Rp <?= number_format($order['total_price'], 0, ',', '.') ?></p>
                        </div>

                        <div class="flex gap-3 w-full md:w-auto flex-wrap justify-end">
                            <a href="invoice.php?ids=<?= $order['id'] ?>&pay=<?= $order['total_price'] ?>" class="flex-1 md:flex-none px-5 py-2.5 bg-white border border-[var(--border-color)] text-[var(--deep-forest)] font-bold rounded-xl hover:bg-gray-50 transition-all text-sm flex items-center justify-center gap-2">
                                <span class="material-symbols-outlined text-sm">print</span> Struk
                            </a>

                            <?php if($need_upload): ?>
                                <button onclick="openUploadModal(<?= $order['id'] ?>, '<?= $order['invoice_number'] ?>')" class="flex-1 md:flex-none px-6 py-2.5 bg-[var(--deep-forest)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] transition-all text-sm flex items-center justify-center gap-2 animate-pulse">
                                    <span class="material-symbols-outlined text-sm">upload_file</span> Upload Bukti
                                </button>
                            <?php endif; ?>

                            <?php if($order['status'] == 'rejected'): ?>
                                <form method="POST" onsubmit="return confirm('Ajukan pengembalian dana?')" class="flex-1 md:flex-none">
                                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                    <button type="submit" name="request_refund" class="w-full px-6 py-2.5 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-all text-sm flex items-center justify-center gap-2">
                            ...
                            <?php if($order['status'] == 'refund'): 
                                $wa_text = "Halo Admin, saya ingin konfirmasi refund untuk Invoice: " . $order['invoice_number'];
                            ?>
                                <a href="https://wa.me/628123456789?text=<?= urlencode($wa_text) ?>" target="_blank" class="flex-1 md:flex-none px-6 py-2.5 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition-all text-sm flex items-center justify-center gap-2">
                                    <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/WhatsApp.svg" class="w-4 h-4"> Hubungi Admin
                                </a>
                                
                                <form method="POST" onsubmit="return confirm('Pastikan Anda sudah menerima dana refund sebelum menekan tombol ini. Lanjutkan?')" class="flex-1 md:flex-none">
                                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                    <button type="submit" name="confirm_refund" class="w-full px-6 py-2.5 bg-stone-600 text-white font-bold rounded-xl hover:bg-stone-700 transition-all text-sm flex items-center justify-center gap-2">
                                        <span class="material-symbols-outlined text-sm">check_circle</span> Uang Diterima
                                    </button>
                                </form>
                            <?php endif; ?>

                            <?php if($order['status'] == 'shipping'): ?>
                                <form method="POST" onsubmit="return confirm('Pesanan sudah diterima?')" class="flex-1 md:flex-none">
                                    <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                    <button type="submit" name="finish_order" class="w-full px-6 py-2.5 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-all text-sm">
                                        Pesanan Diterima
                                    </button>
                                </form>
                            <?php endif; ?>
                    ...
                    <?php if($order['status'] == 'shipping' && !empty($order['tracking_number'])): 
                        $tracking_link = "#";
                    ...
                    ?>
                    <div class="mt-4 pt-4 border-t border-dashed border-gray-200 text-sm flex justify-between items-center bg-yellow-50 p-3 rounded-xl">
                        <div>
                            <span class="font-bold text-[var(--deep-forest)]"><?= $order['expedition_name'] ?></span> - <span class="font-mono"><?= $order['tracking_number'] ?></span>
                        </div>
                        <a href="<?= $tracking_link ?>" target="_blank" class="px-3 py-1 bg-white border border-yellow-200 text-yellow-700 text-xs font-bold rounded-lg hover:bg-yellow-100 flex items-center gap-1">
            ...
            <?php else: ?>
                <div class="text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-[var(--border-color)]">
                    ...
                    <a href="index.php" class="px-6 py-3 bg-[var(--deep-forest)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] transition-all">Belanja Sekarang</a>
                </div>
            <?php endif; ?>
    ...
    <div id="uploadModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-[70]">
        ...
        <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-[2rem] shadow-2xl z-50 p-8 transform scale-95 transition-all">
            ...
            <p class="text-sm text-gray-500 mb-6">Invoice: <span id="modalInvoice" class="font-bold"></span></p>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-[10px] text-gray-400 mt-2">*Format: JPG, PNG. Maks 2MB.</p>
                </div>
                
                <div class="flex gap-3">
                    <button type="button" onclick="toggleModal('uploadModal')" class="flex-1 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-200">Batal</button>
                    <button type="submit" name="upload_proof" class="flex-1 py-3 bg-[var(--deep-forest)] text-white font-bold rounded-xl hover:opacity-90 flex items-center justify-center gap-2">
