...
if (isset($_POST['add_category'])) {
    ...
    if(mysqli_num_rows($check) > 0){
        $alert = "<script>alert('Gagal: Nama kategori sudah ada!');</script>";
    } else {
        mysqli_query($conn, "INSERT INTO categories (name, description) VALUES ('$name', '$desc')");
        $alert = "<script>alert('Kategori Berhasil Ditambahkan!'); window.location='categories.php';</script>";
    }
}
...
if (isset($_GET['delete'])) {
    ...
    // Cek apakah kategori dipakai di buku?
    $check_book = mysqli_query($conn, "SELECT id FROM books WHERE category_id = '$id'");
    if(mysqli_num_rows($check_book) > 0){
        $alert = "<script>alert('Gagal: Kategori sedang digunakan oleh produk buku. Hapus buku terkait terlebih dahulu!'); window.location='categories.php';</script>";
    } else {
        mysqli_query($conn, "DELETE FROM categories WHERE id = '$id'");
        $alert = "<script>alert('Kategori Berhasil Dihapus!'); window.location='categories.php';</script>";
    }
}
...
if (isset($_POST['edit_category'])) {
    ...
    $alert = "<script>alert('Kategori Berhasil Diperbarui!'); window.location='categories.php';</script>";
}
...
<title>Kelola Kategori - Libraria Admin</title>
    ...
    <main id="main-content" class="flex-1 ml-64 p-4 lg:p-8 transition-all duration-300">
        ...
        <header class="flex justify-between items-center mb-10 bg-white/50 dark:bg-stone-800/50 backdrop-blur-sm p-4 rounded-3xl border border-[var(--border-color)] dark:border-stone-700 sticky top-4 z-20 shadow-sm" data-aos="fade-down">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl lg:text-2xl title-font text-[var(--text-dark)] dark:text-stone-200 hidden md:block">Kategori Buku</h2>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4 relative">
                ...
                <button onclick="toggleModal('addCategoryModal')" class="hidden md:flex items-center gap-2 px-5 py-2.5 bg-[var(--chocolate-brown)] text-white font-bold rounded-2xl hover:opacity-90 transition-all shadow-lg shadow-orange-900/20 text-sm">
        ...
        <div class="bg-white dark:bg-stone-800 rounded-[2.5rem] p-8 border border-[var(--border-color)] dark:border-stone-700 card-shadow" data-aos="fade-up">
            <h3 class="text-xl font-bold title-font mb-6 text-[var(--text-dark)] dark:text-stone-200">Daftar Kategori</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <?php while($cat = mysqli_fetch_assoc($categories)):
                    // Count books per category
                    $cat_id = $cat['id'];
                    $q_count = mysqli_query($conn, "SELECT COUNT(*) as total FROM books WHERE category_id = '$cat_id'");
                    $book_count = mysqli_fetch_assoc($q_count)['total'];
                ?>
                <div class="p-6 rounded-[2rem] border border-[var(--border-color)] dark:border-stone-700 hover:shadow-lg transition-all group bg-[var(--cream-bg)]/30 dark:bg-stone-700/30">
                    <div class="flex justify-between items-start mb-4">
                        <div class="w-12 h-12 rounded-2xl bg-white dark:bg-stone-600 flex items-center justify-center text-[var(--deep-forest)] dark:text-[var(--warm-tan)] shadow-sm">
                            <span class="material-symbols-outlined text-2xl">menu_book</span>
                        </div>
                        <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onclick="openEditCategory('<?= $cat['id'] ?>', '<?= htmlspecialchars($cat['name']) ?>', '<?= htmlspecialchars($cat['description']) ?>')" class="w-8 h-8 rounded-full bg-stone-200 dark:bg-stone-500 flex items-center justify-center text-stone-500 dark:text-stone-200 hover:bg-[var(--warm-tan)] hover:text-white transition-colors">
                                <span class="material-symbols-outlined text-sm">edit</span>
                            </button>
                            <a href="?delete=<?= $cat['id'] ?>" onclick="return confirm('Hapus kategori ini?')" class="w-8 h-8 rounded-full bg-stone-200 dark:bg-stone-500 flex items-center justify-center text-stone-500 dark:text-stone-200 hover:bg-red-500 hover:text-white transition-colors">
                                <span class="material-symbols-outlined text-sm">delete</span>
                            </a>
                        </div>
                    </div>
                    <h4 class="font-bold text-lg text-[var(--text-dark)] dark:text-stone-200 mb-1"><?= $cat['name'] ?></h4>
                    <p class="text-xs text-[var(--text-muted)] dark:text-stone-400 mb-4 line-clamp-2 min-h-[2.5em]"><?= $cat['description'] ? $cat['description'] : 'Tidak ada deskripsi.' ?></p>
                    <div class="pt-4 border-t border-dashed border-[var(--border-color)] dark:border-stone-600 flex justify-between items-center">
                        <span class="text-[10px] font-bold uppercase text-[var(--text-muted)] dark:text-stone-400 tracking-wider">Total Produk</span>
                        <span class="text-sm font-bold text-[var(--deep-forest)] dark:text-[var(--warm-tan)]"><?= $book_count ?></span>
                    </div>
                </div>
                <?php endwhile; ?>
            </div>
...
<div id="addCategoryModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 transition-opacity duration-300">
    <div class="modal-overlay absolute w-full h-full bg-stone-900/60 backdrop-blur-sm" onclick="toggleModal('addCategoryModal')"></div>
    <div class="modal-container bg-white dark:bg-stone-800 w-11/12 md:max-w-md mx-auto rounded-[2rem] shadow-2xl z-50 overflow-y-auto p-8 transform transition-all scale-95 duration-300">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-[var(--deep-forest)] dark:text-[var(--warm-tan)] title-font">Tambah Kategori</h3>
            <button onclick="toggleModal('addCategoryModal')" class="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 hover:bg-red-100 hover:text-red-500 transition-colors"><span class="material-symbols-outlined text-lg">close</span></button>
        </div>
        <form action="" method="POST" class="space-y-4">
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] dark:text-stone-400 mb-1.5 ml-1">Nama Kategori</label>
                <input type="text" name="name" required class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] dark:bg-stone-700 border-transparent focus:bg-white dark:focus:bg-stone-600 focus:border-[var(--warm-tan)] focus:ring-0 transition-all text-sm">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] dark:text-stone-400 mb-1.5 ml-1">Deskripsi Singkat</label>
                <textarea name="description" rows="3" class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] dark:bg-stone-700 border-transparent focus:bg-white dark:focus:bg-stone-600 focus:border-[var(--warm-tan)] focus:ring-0 transition-all text-sm"></textarea>
            </div>
            <button type="submit" name="add_category" class="w-full py-4 mt-4 bg-[var(--deep-forest)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] transition-all shadow-lg">Simpan Kategori</button>
        </form>
    </div>
</div>
...
<div id="editCategoryModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 transition-opacity duration-300">
    ...
    <div class="modal-container bg-white dark:bg-stone-800 w-11/12 md:max-w-md mx-auto rounded-[2rem] shadow-2xl z-50 overflow-y-auto p-8 transform transition-all scale-95 duration-300">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-[var(--deep-forest)] dark:text-[var(--warm-tan)] title-font">Edit Kategori</h3>
            <button onclick="toggleModal('editCategoryModal')" class="w-8 h-8 rounded-full bg-stone-100 dark:bg-stone-700 flex items-center justify-center text-stone-400 hover:bg-red-100 hover:text-red-500 transition-colors"><span class="material-symbols-outlined text-lg">close</span></button>
        </div>
        <form action="" method="POST" class="space-y-4">
            <input type="hidden" name="id" id="edit_cat_id">
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] dark:text-stone-400 mb-1.5 ml-1">Nama Kategori</label>
                <input type="text" name="name" id="edit_cat_name" required class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] dark:bg-stone-700 border-transparent focus:bg-white dark:focus:bg-stone-600 focus:border-[var(--warm-tan)] focus:ring-0 transition-all text-sm">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] dark:text-stone-400 mb-1.5 ml-1">Deskripsi Singkat</label>
                <textarea name="description" id="edit_cat_desc" rows="3" class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] dark:bg-stone-700 border-transparent focus:bg-white dark:focus:bg-stone-600 focus:border-[var(--warm-tan)] focus:ring-0 transition-all text-sm"></textarea>
            </div>
            <button type="submit" name="edit_category" class="w-full py-4 mt-4 bg-[var(--warm-tan)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] transition-all shadow-lg">Update Kategori</button>
        </form>
    </div>
</div>
