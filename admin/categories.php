...
if (isset($_POST['add_category'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        $query = "INSERT INTO categories (name, created_by) VALUES ('$name', '$created_by')";
...
if (isset($_POST['edit_category'])) {
    ...
    $check = mysqli_query($conn, "SELECT * FROM categories WHERE name = '$name' AND id != '$id'");
    if (mysqli_num_rows($check) > 0) {
        $alert = "<script>alert('GAGAL: Nama kategori sudah digunakan!');</script>";
    } else {
        $query = "UPDATE categories SET name='$name' WHERE id='$id'";
        mysqli_query($conn, $query);
        $alert = "<script>alert('Kategori Diperbarui!'); window.location='categories.php';</script>";
    }
}

// 4. HAPUS KATEGORI (Proteksi Relasi)
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    
    // Cek apakah ada buku di kategori ini
    $check_books = mysqli_query($conn, "SELECT COUNT(*) as total FROM books WHERE category_id='$id'");
    $book_data = mysqli_fetch_assoc($check_books);
    
    if ($book_data['total'] > 0) {
        $alert = "<script>alert('GAGAL: Kategori tidak bisa dihapus karena masih memiliki " . $book_data['total'] . " buku!'); window.location='categories.php';</script>";
    } else {
...
// AMBIL DATA KATEGORI
$query_cats = "SELECT c.*, u.full_name as creator_name, COUNT(b.id) as total_books 
               FROM categories c 
               LEFT JOIN users u ON c.created_by = u.id 
               LEFT JOIN books b ON c.id = b.category_id 
               GROUP BY c.id 
               ORDER BY c.name ASC";
$categories = mysqli_query($conn, $query_cats);
?>
    ...
    <main id="main-content" class="flex-1 ml-64 p-4 lg:p-8 transition-all duration-300">
        ...
        <header class="flex justify-between items-center mb-10 bg-white/50 backdrop-blur-sm p-4 rounded-3xl border border-[var(--border-color)] sticky top-4 z-20 shadow-sm" data-aos="fade-down">
            <div class="flex items-center gap-4">
                ...
                <div><h2 class="text-xl lg:text-2xl title-font text-[var(--text-dark)] hidden md:block">Kategori Buku</h2></div>
            </div>
        ...
        <button onclick="toggleModal('addCategoryModal')" class="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-[var(--chocolate-brown)] text-white rounded-full shadow-2xl flex items-center justify-center z-40 hover:scale-110 transition-transform active:scale-90">
            <span class="material-symbols-outlined text-2xl">add</span>
        </button>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8" data-aos="fade-up" data-aos-delay="50">
            <div class="bg-white p-6 rounded-[2rem] border border-[var(--border-color)] card-shadow flex items-center gap-4">
                <div class="w-12 h-12 rounded-full bg-[var(--light-sage)]/40 flex items-center justify-center text-[var(--deep-forest)]">
                    <span class="material-symbols-outlined">category</span>
                </div>
                <div>
                    <h4 class="text-2xl font-bold text-[var(--deep-forest)]"><?= mysqli_num_rows($categories) ?></h4>
                    <p class="text-xs text-[var(--text-muted)] font-bold uppercase">Total Kategori</p>
                </div>
            </div>
        </div>

        <section class="bg-white rounded-[2.5rem] p-6 lg:p-8 border border-[var(--border-color)] card-shadow overflow-hidden" data-aos="fade-up" data-aos-delay="100">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-[var(--text-muted)] text-sm border-b border-[var(--border-color)] bg-stone-50/50">
                            <th class="p-4 pl-6 font-bold uppercase tracking-wider w-12">No</th>
                            <th class="p-4 font-bold uppercase tracking-wider">Nama Kategori</th>
                            <th class="p-4 font-bold uppercase tracking-wider">Dibuat Oleh</th>
                            <th class="p-4 font-bold uppercase tracking-wider text-center">Jumlah Buku</th>
                            <th class="p-4 font-bold uppercase tracking-wider text-center pr-6">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-[var(--text-dark)]">
                        ...
                        ?>
                        <tr class="border-b border-[var(--border-color)] hover:bg-[var(--cream-bg)]/50 transition-colors group">
                            
                            <td class="p-4 pl-6 font-bold text-stone-400">
                                <?= $no++ ?>
                            </td>

                            <td class="p-4">
                                <span class="font-bold text-lg text-[var(--deep-forest)]"><?= $cat['name'] ?></span>
                            </td>

                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    <span class="material-symbols-outlined text-[16px] text-stone-400">admin_panel_settings</span>
                                    <span class="text-stone-600 font-medium"><?= $cat['creator_name'] ?: 'System' ?></span>
                                </div>
                            </td>

                            <td class="p-4 text-center">
                                <?php if($cat['total_books'] > 0): ?>
                                    <span class="inline-flex items-center gap-1 bg-green-50 text-green-700 px-3 py-1 rounded-full text-xs font-bold border border-green-100">
                                        <?= $cat['total_books'] ?> Buku
                                    </span>
                                <?php else: ?>
                                    <span class="inline-flex items-center gap-1 bg-stone-50 text-stone-400 px-3 py-1 rounded-full text-xs font-bold border border-stone-200">
                                        0 Buku
                                    </span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="p-4 text-center pr-6">
                                <div class="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onclick="openEditModal(<?= htmlspecialchars(json_encode($cat)) ?>)" 
                                            class="w-8 h-8 rounded-lg flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 hover:scale-110 transition-all shadow-sm" title="Edit">
                                        <span class="material-symbols-outlined text-lg">edit</span>
                                    </button>
                                    
                                    <?php if($cat['total_books'] == 0): ?>
                                    <a href="?delete=<?= $cat['id'] ?>" onclick="return confirm('Hapus kategori ini?')" 
                                       class="w-8 h-8 rounded-lg flex items-center justify-center bg-red-50 text-red-600 hover:bg-red-100 hover:scale-110 transition-all shadow-sm" title="Hapus">
                                        <span class="material-symbols-outlined text-lg">delete</span>
                                    </a>
                                    <?php else: ?>
                                    <button onclick="alert('Tidak dapat menghapus! Kategori ini masih digunakan oleh <?= $cat['total_books'] ?> buku.')" 
                                            class="w-8 h-8 rounded-lg flex items-center justify-center bg-stone-100 text-stone-300 cursor-not-allowed" title="Terpakai">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="p-8 text-center text-stone-400 italic">Belum ada kategori buku.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
...
<div id="addCategoryModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 transition-opacity duration-300">
    ...
    <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-[2rem] shadow-2xl z-50 overflow-y-auto p-8 transform transition-all scale-95 duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-[var(--deep-forest)] title-font">Tambah Kategori</h3>
            <button onclick="toggleModal('addCategoryModal')" class="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-stone-400 hover:bg-red-100 hover:text-red-500 transition-colors">
        ...
        <form action="" method="POST" class="space-y-4">
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] mb-1.5 ml-1">Nama Kategori</label>
                <input type="text" name="name" required placeholder="Contoh: Fiksi Ilmiah" class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] border border-transparent focus:bg-white focus:border-[var(--warm-tan)] focus:ring-0 transition-all">
            </div>
            
            <button type="submit" name="add_category" class="w-full py-4 mt-4 bg-[var(--deep-forest)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] hover:-translate-y-1 transition-all shadow-lg shadow-green-900/20">
                Simpan Kategori
            </button>
        </form>
    </div>
...
<div id="editCategoryModal" class="modal opacity-0 pointer-events-none fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 transition-opacity duration-300">
    ...
    <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-[2rem] shadow-2xl z-50 overflow-y-auto p-8 transform transition-all scale-95 duration-300">
        ...
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-[var(--deep-forest)] title-font">Edit Kategori</h3>
            <button onclick="toggleModal('editCategoryModal')" class="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center text-stone-400 hover:bg-red-100 hover:text-red-500 transition-colors">
                <span class="material-symbols-outlined text-lg">close</span>
            </button>
        </div>
        ...
        <form action="" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase text-[var(--text-muted)] mb-1.5 ml-1">Nama Kategori</label>
                <input type="text" name="name" id="edit_name" required class="w-full px-4 py-3 rounded-xl bg-[var(--cream-bg)] border border-transparent focus:bg-white focus:border-[var(--warm-tan)] focus:ring-0 transition-all">
            </div>
            
            <button type="submit" name="edit_category" class="w-full py-4 mt-4 bg-[var(--warm-tan)] text-white font-bold rounded-xl hover:bg-[var(--chocolate-brown)] hover:-translate-y-1 transition-all shadow-lg shadow-orange-900/20">
                Update Kategori
            </button>
        </form>
    </div>
